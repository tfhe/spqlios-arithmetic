...
void reim4_add_1blk_to_reim_avx(uint64_t m, uint64_t blk,
  ...
  __m256d s = _mm256_loadu_pd(src_ptr);
  __m256d d = _mm256_loadu_pd(dst_ptr);
  _mm256_storeu_pd(dst_ptr, _mm256_add_pd(s, d));

  __m256d s = _mm256_loadu_pd(src_ptr + 4);
  __m256d d = _mm256_loadu_pd(dst_ptr + m);
  _mm256_storeu_pd(dst_ptr + m, _mm256_add_pd(s, d));
}
