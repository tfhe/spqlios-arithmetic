...
EXPORT void reim_fftvec_sub_fma(const REIM_FFTVEC_ADD_PRECOMP* precomp, double* r, const double* a, const double* b) {
  const uint64_t m = precomp->m;
  double* rr_ptr = r;
  double* ri_ptr = r + m;
  const double* ar_ptr = a;
  ...
  while (rr_ptr != rend_ptr) {
    ...
    __m256d rr = _mm256_sub_pd(ar, br);
    __m256d ri = _mm256_sub_pd(ai, bi);
