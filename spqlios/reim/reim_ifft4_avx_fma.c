...
EXPORT void reim_ifft4_avx_fma(double* dre, double* dim, const void* ompv) {
  ...
  // 1
  {
    ...
    ra01 = _mm_unpacklo_pd(ra, rb);
    ia01 = _mm_unpacklo_pd(ia, ib);
    ra23 = _mm_unpackhi_pd(ra, rb);
    ia23 = _mm_unpackhi_pd(ia, ib);
  }
  ...
  // 2
  {
    ...
    reim_invctwiddle_avx_fma(&ra01, &ra23, &ia01, &ia23, omre, omim);
  }

  // 4
  _mm_storeu_pd(dre, ra01);
  _mm_storeu_pd(dre + 2, ra23);
  _mm_storeu_pd(dim, ia01);
  _mm_storeu_pd(dim + 2, ia23);
}
