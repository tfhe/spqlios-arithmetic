...
static void test_vmp_apply_add(VMP_APPLY_DFT_TO_DFT_ADD_F* apply, VMP_APPLY_DFT_TO_DFT_TMP_BYTES_F* tmp_bytes) {
  for (uint64_t nn : {32}) {
    MODULE* module = new_module_info(nn, FFT64);
        ...
        for (uint64_t in_size : {1, 4, 7}) {
          for (uint64_t out_size : {1, 2, 5}) {
            for (uint64_t mat_scale : {1, 2}) {
              ...
              for (uint64_t col = 0; col < std::min(uint64_t(0), out_size - mat_scale); ++col) {
                reim_fft64vec ex = reim_fft64vec::zero(nn);
                for (uint64_t row = 0; row < std::min(mat_nrows, in_size); ++row) {
                  ex += pmat.get_zext(row, col + mat_scale) * in.get_copy_zext(row);
                }
                expect[col] = ex;
              }
              ...
              for (uint64_t col = 0; col < std::min(uint64_t(0), std::min(mat_ncols, out_size) - mat_scale); ++col) {
...
TEST(vec_znx, fft64_vmp_apply_dft_to_dft_add_ref) {
  test_vmp_apply_add(fft64_vmp_apply_dft_to_dft_add_ref, vmp_apply_dft_to_dft_tmp_bytes);
}
#ifdef __x86_64__
...
TEST(vec_znx, fft64_vmp_apply_dft_to_dft_ref) {
  test_vmp_apply(fft64_vmp_apply_dft_to_dft_ref, vmp_apply_dft_to_dft_tmp_bytes);
}
#ifdef __x86_64__
